...
##  ⁉️ Принцип работы
1. Пользователь делает фото/снимает объект на видео через мобильный телефон. Система распознает изображение и отправляет данные на сервер
2. На сервере имеется предобученная модель Resnet50, содержащая, на данный момент, 41 объект (класс) для сопоставления.
3. Модель распознает объект, сервер отправляет результат пользователю.
4. Пользователь может написать вопрос в текстовом поле, например, "РАсскажи про Венеру Таврическую". Вопрос отправляется на сервер.
5. На сервере имеется GigaChat, который распознает вопрос и ищет наиболее релевантный ответ среди имеющегося массива текста.
6. Результат направляется пользователю.

Схема работы:

## 💾 Как настроить  
1. Скачайте проект.
2. В папке с проектом создайте виртуальное окружение
